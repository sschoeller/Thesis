...
# bit-based with Levenshtein distances
    ...
    def search(self):
        ...
        for j in range(0, len(S)-3): # len(S) - 4 end 
            ...
            if self.binList[pos] >> (j*4) & 0x00000007 % 7 == 0: # Check for all codons due to false negative results on "TAA"
                if td.levenshtein(Codon, "TA") == 1 or td.levenshtein(Codon, "TG") == 1:
                    self.stopList.append(j)
                    binShift += 1
                elif td.levenshtein(Codon, "AT") == 1:
                    self.startList.append(j)
                    binShift += 1
                else:
                    binShift += 1
            elif self.binList[pos] >> (j*4) & 0x00000003 % 3 == 0 and td.levenshtein(Codon, "TA") == 1:
                self.stopList.append(j)
                binShift += 1
            else:
